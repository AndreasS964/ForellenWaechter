# 📡 ForellenWächter v1.4 - LTE Remote Edition

Vollständige Anleitung für die LTE-Version mit Remote-Zugriff und E-Mail-Alarmen.

---

## 🆕 Neu in Version 1.4

| Feature | Beschreibung |
|---------|--------------|
| 📡 **LTE/4G Mobilfunk** | Remote-Zugriff von überall |
| 📧 **E-Mail Alarme** | Sofortige Benachrichtigung bei Problemen |
| 🫧 **DO-Sensor Support** | Dissolved Oxygen (Sauerstoff) optional |
| 🎨 **Neues Dashboard** | Modernes Glassmorphism-Design |
| ⏰ **Echtzeit-Uhr** | NTP-Synchronisation über Internet |
| 📊 **Erweiterte API** | Mehr Endpunkte, Settings-POST |

---

## 🔌 Hardware v1.4

### Neue Komponenten

| Komponente | Beschreibung | Bezugsquelle | ~Preis |
|------------|--------------|--------------|--------|
| LILYGO T-SIM7600E | ESP32 mit LTE-Modul integriert | AliExpress | 45€ |
| **ODER** SIM7600E Modul | Separates LTE-Modul | Amazon | 35€ |
| SIM-Karte | Daten-SIM mit min. 100MB/Monat | Provider | 5€/Monat |
| DFRobot Gravity DO | Dissolved Oxygen Sensor (optional) | DFRobot | 80€ |
| Externe Antenne | LTE Antenne (empfohlen für Wald) | Amazon | 15€ |

### Pin-Belegung v1.4

```
ESP32 GPIO    Funktion
──────────────────────────────────────
GPIO 4        DS18B20 OneWire Bus
GPIO 34       pH-Sensor (ADC)
GPIO 35       TDS-Sensor (ADC)
GPIO 32       DO-Sensor (ADC) - NEU!
GPIO 33       Float Switch

GPIO 26       Relais 1 - Belüftung
GPIO 27       Relais 2 - Alarm
GPIO 14       Relais 3 - Reserve
GPIO 12       Relais 4 - Reserve

GPIO 2        Status LED (blau)
GPIO 13       Alarm LED (rot) - NEU!
GPIO 15       Buzzer

GPIO 5        SD-Karte CS
GPIO 23       SD-Karte MOSI
GPIO 19       SD-Karte MISO
GPIO 18       SD-Karte SCK

GPIO 17       LTE TX (SIM7600)
GPIO 16       LTE RX (SIM7600)
```

---

## 📱 LTE Einrichtung

### 1. SIM-Karte vorbereiten

- **Empfehlung:** IoT/M2M SIM-Karte
- **Datenvolumen:** Min. 100MB/Monat (reicht für Monitoring)
- **PIN deaktivieren!** (wichtig)

Provider-Empfehlungen (DE):
- **1NCE:** 10€ einmalig, 500MB für 10 Jahre
- **Telekom IoT:** Ab 2€/Monat
- **Congstar Prepaid:** Flexibel

### 2. APN konfigurieren

```cpp
// Im Code anpassen:
#define LTE_APN "internet"       // Telekom
// #define LTE_APN "web.vodafone.de"  // Vodafone
// #define LTE_APN "internet.t-mobile" // T-Mobile
// #define LTE_APN "pinternet.interkom.de" // O2
```

### 3. LILYGO T-SIM7600E Board

Falls du das integrierte Board verwendest:

```cpp
// Pin-Definitionen für LILYGO T-SIM7600E:
#define LTE_TX 27
#define LTE_RX 26
#define LTE_PWR 4
#define LTE_RST 5
#define LTE_DTR 32
```

### 4. Verbindung testen

Nach Upload → Serial Monitor:
```
📡 LTE wird initialisiert...
   Modul: SIMCOM SIM7600E
✅ LTE verbunden!
   Operator: Telekom.de
   Signal: 18/31
```

---

## 📧 E-Mail Alarme einrichten

### Option 1: IFTTT Webhook (Empfohlen)

1. **IFTTT Account erstellen:** https://ifttt.com
2. **Neues Applet erstellen:**
   - IF: Webhooks → "Receive a web request"
   - Event Name: `forellen_alarm`
   - THEN: Email → "Send me an email"
3. **Webhook URL kopieren:**
   - Gehe zu: https://ifttt.com/maker_webhooks
   - "Documentation" klicken
   - URL kopieren

4. **Im Code eintragen:**
```cpp
const char* EMAIL_WEBHOOK_URL = "https://maker.ifttt.com/trigger/forellen_alarm/with/key/DEIN_KEY";
```

### Option 2: Eigener Server

Falls du einen eigenen Webhook-Server hast:

```cpp
const char* EMAIL_WEBHOOK_URL = "https://dein-server.de/api/forellen-alert";
```

Der Server sollte JSON empfangen:
```json
{
  "value1": "🚨 ForellenWächter ALARM",
  "value2": "Temperatur KRITISCH: 16.5°C\n...",
  "value3": "2024-12-01 14:30:00"
}
```

### E-Mail Einstellungen

```cpp
const char* EMAIL_RECIPIENT = "lucas@example.com";
#define EMAIL_COOLDOWN_MIN 30  // Min. 30 Minuten zwischen E-Mails
```

---

## 🫧 Sauerstoff-Sensor (DO)

### DFRobot Gravity Dissolved Oxygen Sensor

**Aktivierung:**
```cpp
#define ENABLE_DO_SENSOR true
```

**Verkabelung:**
```
DO-Sensor           ESP32
──────────────────────────
VCC          →      3.3V
GND          →      GND
Signal       →      GPIO 32
```

**Kalibrierung (wichtig!):**

1. **Einpunkt-Kalibrierung (Luft):**
   - Sensor-Spitze in die Luft halten (NICHT ins Wasser)
   - Warten bis Wert stabil
   - Spannung notieren (ca. 1.5V bei 20°C)
   - Im Code anpassen:
   ```cpp
   const float V_SATURATION = 1.5;  // Dein Wert
   ```

2. **Zweipunkt-Kalibrierung (genauer):**
   - Zusätzlich Nullpunkt mit Natriumsulfit-Lösung

**Grenzwerte für Forellen:**
```cpp
float doMin = 6.0;     // Minimum (Alarm wenn unterschritten)
float doOptimal = 9.0; // Optimal (Belüftung aktiviert wenn darunter)
```

---

## 🎨 Neues Dashboard

### Features

- **Glassmorphism Design** - Moderne, durchscheinende UI
- **Echtzeit-Updates** - Alle 2 Sekunden
- **Responsive** - Optimiert für Handy und Desktop
- **Animationen** - Schwimmender Fisch, pulsierende Alarme
- **Dunkles Theme** - Angenehm für die Augen

### Zugriff

- **Lokal:** http://192.168.4.1 oder http://forellenwaechter.local
- **Remote:** Über öffentliche IP (wenn Router konfiguriert)

### Screenshots

```
┌──────────────────────────────────────────────────────────────┐
│                        🐟                                    │
│                  FORELLENWÄCHTER                             │
│        Lucas Haug Forellenzucht • Powered by Wasserkraft ⚡   │
├──────────────────────────────────────────────────────────────┤
│  ○ WiFi: -45 dBm    ○ LTE: 18/31    ○ Uptime: 2d 5h         │
├──────────────────────────────────────────────────────────────┤
│ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐      │
│ │  💧  │ │  🌡️  │ │  🧪  │ │  📊  │ │  🫧  │ │  🌊  │      │
│ │ 11.5 │ │ 18.2 │ │ 7.24 │ │ 185  │ │ 8.5  │ │  OK  │      │
│ │  °C  │ │  °C  │ │      │ │ ppm  │ │ mg/L │ │      │      │
│ └──────┘ └──────┘ └──────┘ └──────┘ └──────┘ └──────┘      │
├──────────────────────────────────────────────────────────────┤
│ 📈 Temperaturverlauf                    [24h] [7d]          │
│ ╭─────────────────────────────────────────────╮             │
│ │   ─── Wasser ─── Luft                       │             │
│ │     ╱╲      ╱╲      ╱╲                     │             │
│ ╰─────────────────────────────────────────────╯             │
└──────────────────────────────────────────────────────────────┘
```

---

## 🔧 Konfiguration

### Feature-Toggles

```cpp
#define ENABLE_LTE true           // LTE Mobilfunk
#define ENABLE_WIFI true          // WiFi (parallel zu LTE)
#define ENABLE_EMAIL_ALERTS true  // E-Mail Benachrichtigungen
#define ENABLE_DO_SENSOR false    // Dissolved Oxygen
#define ENABLE_SD_LOGGING true    // SD-Karten Logging
```

### Grenzwerte anpassen

Im Code oder über API:

```cpp
struct TroutParameters {
  float tempMin = 8.0;       // Minimum Wassertemperatur
  float tempMax = 14.0;      // Maximum Wassertemperatur
  float tempCritical = 16.0; // Kritische Temperatur
  float phMin = 6.5;         // Minimum pH
  float phMax = 8.5;         // Maximum pH
  float tdsMax = 500;        // Maximum TDS
  float doMin = 6.0;         // Minimum Sauerstoff
  float doOptimal = 9.0;     // Optimaler Sauerstoff
};
```

Oder per POST:
```bash
curl -X POST http://192.168.4.1/api/settings \
  -H "Content-Type: application/json" \
  -d '{"tempMax": 15.0, "phMin": 6.0}'
```

---

## 📡 Remote-Zugriff einrichten

### Option 1: Port-Forwarding (Heimnetz mit Router)

1. Router-Einstellungen öffnen
2. Port-Forwarding: Port 80 → ESP32 IP
3. DynDNS einrichten für feste Domain

### Option 2: VPN (sicherer)

- WireGuard auf Raspberry Pi
- Zugriff über VPN-Tunnel

### Option 3: Cloud-Dashboard (zukünftig)

- Daten an Cloud senden
- Dashboard überall verfügbar
- Geplant für v1.5

---

## 📊 API v1.4

### Neue Endpunkte

| Endpunkt | Methode | Beschreibung |
|----------|---------|--------------|
| `/api/sensors` | GET | Alle Sensorwerte inkl. DO |
| `/api/status` | GET | System + LTE Status |
| `/api/settings` | GET | Aktuelle Grenzwerte |
| `/api/settings` | POST | Grenzwerte ändern |
| `/api/test-email` | POST | Test-E-Mail senden |

### Beispiel: Status

```bash
curl http://192.168.4.1/api/status
```

```json
{
  "uptime": 86400,
  "freeHeap": 180000,
  "wifiConnected": true,
  "wifiRSSI": -45,
  "lteConnected": true,
  "lteSignal": 18,
  "lteOperator": "Telekom.de",
  "publicIP": "93.xxx.xxx.xxx",
  "sdCard": true,
  "alarmCount": 3,
  "dailyAlarms": 1,
  "firmware": "1.4.0"
}
```

---

## 🐛 Troubleshooting LTE

### LTE verbindet nicht

1. **SIM-PIN deaktiviert?**
   - SIM in Handy testen
   - PIN in Handy-Einstellungen ausschalten

2. **APN korrekt?**
   - Provider-APN prüfen
   - Im Code anpassen

3. **Antenne angeschlossen?**
   - Externe Antenne verwenden
   - Position optimieren (Fenster)

4. **Signal zu schwach?**
   - CSQ < 10 = problematisch
   - Antenne mit Kabel verlängern

### E-Mail kommt nicht an

1. **Webhook-URL korrekt?**
   - In Browser testen
   - IFTTT Applet aktiviert?

2. **Internet-Verbindung?**
   - LTE oder WiFi verbunden?
   - `/api/status` prüfen

3. **Cooldown aktiv?**
   - Standard: 30 Min zwischen E-Mails
   - Im Code anpassen

---

## ⚡ Stromversorgung

Mit Wasserkraft hast du genug Power für 24/7 Betrieb!

**Empfohlene Konfiguration:**
- Wasserkraft-Turbine: 200W
- Laderegler: 12V/24V
- Batterie: 100Ah LiFePO4 (Puffer für Niedrigwasser)
- Step-Down: 12V → 5V für ESP32

**Stromverbrauch v1.4:**
- ESP32 + LTE: ~200mA
- Sensoren: ~50mA
- Relais: ~80mA pro aktives Relais
- **Gesamt:** ~300-500mA (1.5-2.5W)

---

## 🔮 Vorbereitungen für v1.5+

Der Code ist bereits vorbereitet für:

- **Multi-Becken:** Slave-Geräte über ESP-NOW
- **Fischdatenbank:** Artspezifische Parameter
- **Cloud-Integration:** MQTT Support
- **OTA Updates:** Over-the-Air Firmware-Updates

Siehe [ROADMAP.md](ROADMAP.md) für Details.
